...
--
-- Add this view to the Genie space alongside the Gold tables.
-- Genie will use it to answer questions like:
--   "What is our current DSO?"
--   "What is the touchless invoice rate?"
--   "Show me all finance KPIs"
--   "How much AP is overdue?"
-- =============================================================

CREATE OR REPLACE VIEW akash_s_demo.finance_and_accounting.gold_finance_kpis
...
p2p AS (
  ...
  FROM akash_s_demo.finance_and_accounting.gold_fact_invoices
),
...
o2c AS (
  SELECT
    -- AR balances (balance_outstanding = amount not yet collected)
  ...
  FROM akash_s_demo.finance_and_accounting.gold_fact_collections
),
...
revenue AS (
  ...
  FROM akash_s_demo.finance_and_accounting.gold_fact_sales
),
...
overdue_customers AS (
  ...
  FROM akash_s_demo.finance_and_accounting.gold_dim_customer
...
gl AS (
  ...
  FROM akash_s_demo.finance_and_accounting.gold_fact_gl
),
...
tb AS (
  ...
  FROM akash_s_demo.finance_and_accounting.gold_fact_trial_balance
  WHERE period = (
    SELECT MAX(period) FROM akash_s_demo.finance_and_accounting.gold_fact_trial_balance
  )
)
